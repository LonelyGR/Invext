...
1. Бот генерирует UUID‑токен и сохраняет его в таблицу `admin_tokens` (логика генерации в боте реализуется отдельно).
2. Токен живёт 24 часа (`expires_at = now + 24h`).
3. Админ открывает `/database`, вводит токен в форму логина.
4. Backend:
   - валидирует токен (`admin_tokens.token`, `expires_at`);
   - создаёт JWT‑сессию и устанавливает httpOnly‑cookie `admin_jwt` на 24 часа;
   - опционально помечает токен как `is_used = true`;
