...
- `/start` и `/start ref_code` — создание/обновление пользователя, привязка реферала.
- Профиль показывает:
  - баланс USDT/USDC (из `GET /v1/wallet/balances`);
  - реферальный код, количество рефералов, оборот команды;
...
- **Расширить инвест‑модуль:**
  - хранить сделки, статусы, доходность;
  - добавить отчётность для пользователя и админки.
